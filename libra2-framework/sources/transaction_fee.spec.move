spec libra2_framework::transaction_fee {
    ...
    spec burn_fee(account: address, fee: u64) {
        ...
        let libra2_addr = type_info::type_of<AptosCoin>().account_address;
    ...
    spec mint_and_refund(account: address, refund: u64) {
        use libra2_std::type_info;
        use libra2_framework::libra2_coin::AptosCoin;
        use libra2_framework::coin::{CoinInfo, CoinStore};
        use libra2_framework::coin;
        // TODO(fa_migration)
        pragma verify = false;
        ...
        aborts_if (refund != 0) && !exists<CoinInfo<AptosCoin>>(libra2_addr);
    ...
    spec store_libra2_coin_burn_cap(libra2_framework: &signer, burn_cap: BurnCapability<AptosCoin>) {
        use std::signer;
    ...
    spec store_libra2_coin_mint_cap(libra2_framework: &signer, mint_cap: MintCapability<AptosCoin>) {
        use std::signer;
        let addr = signer::address_of(libra2_framework);
        aborts_if !system_addresses::is_libra2_framework_address(addr);
